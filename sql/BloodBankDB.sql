...
-- Relationship tables
-- Table for DONATE (Donor to Blood)
CREATE TABLE Donate (
    DID INT,
    Code INT,
    FOREIGN KEY (DID) REFERENCES donors(id),
    FOREIGN KEY (Code) REFERENCES Blood(Code),
    PRIMARY KEY (DID, Code)
);

-- Table for STORED (Blood to BloodBank)
CREATE TABLE Stored_blood (
    Code INT,
    BNO INT,
    FOREIGN KEY (Code) REFERENCES Blood(Code),
    FOREIGN KEY (BNO) REFERENCES BloodBank(BNO),
    PRIMARY KEY (Code, BNO)
);

-- Table for MANAGES (BloodbankManager to BloodBank)
CREATE TABLE Manages (
    Empid INT,
    BNO INT,
    FOREIGN KEY (Empid) REFERENCES BloodbankManager(Empid),
    FOREIGN KEY (BNO) REFERENCES BloodBank(BNO),
    PRIMARY KEY (Empid, BNO)
);

-- Table for ORDERS (Hospital to BloodBank)
CREATE TABLE Orders (
    Name VARCHAR(255),
    BNO INT,
    FOREIGN KEY (Name) REFERENCES Hospital(Name),
    FOREIGN KEY (BNO) REFERENCES BloodBank(BNO),
    PRIMARY KEY (Name, BNO)
);

-- Table for WORKS (Receptionist to BloodBank)
CREATE TABLE Works (
    Empid INT,
    BNO INT,
...
-- Table for REGISTERS (Receptionist to Donor)
